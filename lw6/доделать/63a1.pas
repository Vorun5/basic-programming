...
{DP 1 Сортирует символы, предшествующие #, из INPUT в OUTPUT.
...
BEGIN {SelectSort}
  ...
  DO { Ch <> '#' и Ch1 - первый символ F1}
    BEGIN
      {DP 1.2.1 Выбираем минимальный из F1 b копируем остаток F1 в F2}
      REWRITE(F2);
      Min := Ch;
      READ(F1, Ch);
      WHILE Ch <> '#'
      DO { Ch <> '#' и Ch1 - первый символ F1}
        BEGIN
          {DP 1.2.1.1 Выбираем минимальный из (Ch, Min)
