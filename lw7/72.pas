...
BEGIN {BubbleSort}
  ...
  DO
    BEGIN
      {DP 8.1 Копируем F1 в F2,проверяя отсортированность
      ...
      THEN
        BEGIN
          ...
          DO { По крайней мере два символа остается для Ch1,Ch2 }
            BEGIN
              READ(F1, Ch2);
              {DP 8.1.1 Выводим   min(Ch1,Ch2) в  F2, записывая
          ...
          {DP 8.4 Копируем F1 в OUTPUT. Переносим в цикл.}
          RESET(F1);
          WHILE NOT EOLN(F1)
          DO
            BEGIN
